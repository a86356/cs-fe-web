<template>
  ...
  >
    ...
    >
      ...
      <FormItem
        :label="item1.label"
        :label-width="form.laeblwidth|| 0"
        :prop="item1.name"
        v-for="(item1,index1) in item.fields"
        :key="index1"
        :style="item1.style"
      >
        <div v-if="item1.type=='text'">
          <Input
            ...
            v-model="data[item1.name]"
            :placeholder="item1.placeholder"
            @on-change="dealInputChange($event,item1.name)"
          >
          </Input>
        </div>
        ...
        <div v-if="item1.type=='select'">
          <i-select
            @on-change="dealSelect($event,item1.func,item1.name)"
            :disabled="item1.disabled"
            :placeholder="item1.placeholder"
            v-model="data[item1.name]"
          ...
          >
            ...
            <i-option
              v-for="(item2,index2) in form.data[item1.name+'list']"
              :value="item2.value"
              :key="item2.index2"
        ...
        <div v-if="item1.type=='datetime'">
          <DatePicker
            v-model="data[item1.name]"
            @on-change="data[item1.name]=$event"
            type="datetime"
            :format="item1.format"
            :placeholder="item1.placeholder"
            style="width: 250px"
        ...
        <div v-if="item1.type=='masteriallist'">
          <div class="materiallistwrap">
            <div
              v-for="(item,index) in item1.options.list"
              class="item"
            >
              <div>
                <div class="imgwrap">
                ...
                <div class="tit">
                  ...
                  <RadioGroup
                    v-model="data.fabricId"
                    @on-change="dealEvent(item1.func)"
                  >
        ...
        <div
          v-if="item1.type=='upload'"
          v-show="item1.show?true:false"
        >
          <input
            type="file"
            @change="upload($event)"
...
export default {
  ...
  methods: {
    ...
    uploadpic(e) {
      this.$emit("uploadpic", { ref: this.$refs[this.form.ref], data: e });
    },
    dealSelect(e,func,name){

        if (!func) return;

        this.$emit(func, {value: e.value,name:name,label:e.label });
    },
      dealInputChange(e,a){
          this.$emit('changeinput', {event: e });
    }
  }
};
</script>
